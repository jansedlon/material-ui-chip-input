...
describe("uncontrolled mode", () => {
  it("matches the snapshot", () => {
    const tree = mount(<ChipInput defaultValue={["foo", "bar"]} />);
    expect(tree).toMatchSnapshot();
  });

  it("displays the default value in chips", () => {
    const tree = mount(<ChipInput defaultValue={["foo", "bar", "foobar"]} />);
    expect(tree.find("Chip").map((chip) => chip.text())).toEqual([
      "foo",
      "bar",
      "foobar",
    ]);
  ...
  it("calls onChange when adding new chips", () => {
    ...
    const tree = mount(<ChipInput onChange={handleChange} />);
  ...
  it("calls onUpdateInput when the input changes", () => {
    ...
    tree
      .find("input")
      .simulate("change", { target: tree.find("input").getDOMNode() });
...
describe("controlled mode", () => {
  it("clears the input when the value changes if clearInputValueOnChange is set", () => {
    const tree = mount(<ChipInput value={["foo"]} clearInputValueOnChange />);
  ...
  it("keeps the input when the value changes if clearInputValueOnChange is not set", () => {
    const tree = mount(<ChipInput value={["foo"]} />);
    tree.find("input").simulate("change", { target: { value: "bar" } });
    tree.setProps({ value: ["foo", "bar"] });
...
describe("chip focusing", () => {
  function getFocusedChip(tree) {
    return tree
      .find("Chip")
      .filterWhere((chip) => chip.getDOMNode().style.backgroundColor !== "");
  }

  function focusChip(tree, name) {
    tree
      .find("Chip")
      .filterWhere((chip) => chip.text() === name)
      .simulate("click");
  }
  ...
  it("focuses the last chip when pressing backspace", () => {
    const tree = mount(<ChipInput defaultValue={["a", "b", "c"]} />);
  ...
  it("focuses the last chip when pressing the left arrow key if the input is empty", () => {
    ...
    tree
      .find("input")
      .simulate("keyDown", { keyCode: 37, target: { value: "d" } }); // arrow left
    expect(getFocusedChip(tree).length).toBe(0);
  });
...
describe("placeholder", () => {
  it("displays a placeholder", () => {
    const tree = mount(<ChipInput placeholder="Placeholder" />);

    expect(tree.find("input").getDOMNode().getAttribute("placeholder")).toBe(
      "Placeholder"
    );
  });

  it("is hidden if there are chips", () => {
    const tree = mount(<ChipInput placeholder="Placeholder" value={["foo"]} />);
...
describe("floating label", () => {
  it("is displayed", () => {
    const tree = mount(<ChipInput label="Floating label" />);
...
describe("helper text", () => {
  it("is displayed", () => {
    const tree = mount(<ChipInput helperText="Helper text" />);
...
describe("custom chips", () => {
  it("calls a chip renderer for every chip", () => {
    const chipRenderer = jest.fn(({ text }, key) => (
      <Chip key={key} label={text.toUpperCase()} />
    ));
    const tree = mount(
...
describe("blurBehavior modes", () => {
  it("clears the input on blur", () => {
    const tree = mount(<ChipInput value={["a", "b"]} />);
  ...
  it("does not clear the input on blur with blurBehavior set to ignore", () => {
    const tree = mount(<ChipInput value={["a", "b"]} blurBehavior="ignore" />);
    tree.find("input").simulate("change", { target: { value: "foo" } });
    tree.find("input").simulate("blur");
...
describe("keys", () => {
  ...
  it("stops when onKeyDown prevents the default", () => {
    ...
    const tree = mount(
      <ChipInput onKeyDown={handleKeyDown} newChipKeyCodes={null} /> // This will raise an exception if the code reaches that part
    );
